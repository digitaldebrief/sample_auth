...
original_author="digitaldebrief"
original_name="sample_auth"
original_urlname="sample_auth"
original_description="Awesome sample_auth created by digitaldebrief" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/sample_auth "${name}"
...
then
    ...
    echo "then run 'sample_auth' or 'python -m sample_auth'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'sample_auth' or 'python -m sample_auth' or 'poetry run sample_auth'"
fi
